...
- **The audit sheet**, rendered from the sealed objects by a committed script, one
  ...
  field and observable-from date; the prose targets with their briefs, attempts,
...
| Collision | Each impact's artifact truly collides with the leave: a ticket due inside the leave span and owned by the leaver, a meeting on a leave day the leaver attends, a clause in force over the leave that names the leaver responsible. |
| Verdict reasons | Each must-assess verdict's reasons hold in the plantings: availability is a clashing event or the person's own leave; skill is a record, or a text, lacking it; hard rule is the employment type; component is not being a member. |
| Outcome rule | With V viable and U unknown over the whole organization against the required count n: V at or above n is assign; V plus U below n is uncovered; anything else is unknown. Read off the outcome witness, which must agree with the key. |
...
component. Each class row states exactly which verdicts, unknowns and outcome the
...
approved leave over the same days, and their verdict on every impact they were authored
for must be non-viable on availability alone.
...
against the containment gate, recorded as a question, not as a defect of the text. A
sentence that maps to no predicate of the vocabulary is non-predicate prose, not a recall
failure. Register conformance beyond the fact contract (a component named where the
register forbids it, say) is a prompt-adherence finding at its measured rate, never a
defect of the world.
...
7. **Outcome recomputed** from the witness: viable, unknown, non-viable, the requirement,
   the rule applied mechanically, the people behind an exceptional bucket named. A scan
   of cited rows elsewhere in the sheet is corroboration and never the proof; the
   witness over the rebuilt world is the count.
...
Two disciplines run through every step. Every statement about how a rule behaves is read
in the code at the time of the trace, not remembered, and is labelled implementation
verification where it exceeds what the sheet shows. And the sheet's cited rows are a
projection for the key, never an exhaustive serialization of the world: the witness may
classify a person on a world fact no cited row carries, so whole-sheet counts corroborate
...
- It traces a subset to its evidence; the rest rest on the acceptance pass with the
  outcome rule read off the witness.
...
## 8. Releasing an example

Publishing a scenario's key ends its life as blinded evaluation material. A released
example is first retired: it stays in the audit's provenance and leaves every future
blinded evaluation, recorded as such in the design record before release. The example is
chosen to show the method while leaking the least reusable structure about the scenarios
that remain private, which is not the same as the cleanest trace.

...
def validate(
    ...
    exactness, fidelity = _records_checked(spec, specs, systems)
    exact = {result.check.kind for result in exactness if result.status is CheckStatus.PASSED}
    views = tuple(_view_checked(spec, one, systems, exact) for one in specs)
...
def _cross_check(spec: PlantedWorldSpec, manifest: WorldManifest) -> None:
    recorded = manifest.digest_of(ArtifactRole.TRUTH_MANIFEST)
    if spec.truth_manifest_digest != recorded:
        raise IntegrityRefused(
            "the world spec and the manifest record different truth manifests: "
            f"{spec.truth_manifest_digest} against {recorded}"
        )
...
def _records_checked(
...
) -> tuple[tuple[ExactnessResult, ...], tuple[FidelityResult, ...]]:
    ...
    return tuple(exactness), tuple(fidelity)


def _view_checked(
    spec: PlantedWorldSpec, one: ScenarioSpec, systems: LiveSystems, exact: set[EntityKind]
) -> ViewResult:
    ...
    live = frozenset(
        [
            *derive_live(systems.people.employees(), today),
            *derive_live(systems.work.components(), today),
            *derive_live(systems.work.work_items(), today),
